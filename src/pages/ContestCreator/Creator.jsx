...
const Creator = () => {
    ...
    const handleCreatorApplication = data => {
        ...
        axiosSecure.post('/creators', data)
            .then(res => {
                if (res.data.insertedId) {
                    Swal.fire({
                        position: "top-end",
                        icon: "success",
                        title: "Application submitted successfully. Review may take up to 14 days.",
    ...
    return (
        <div className='w-11/12 mx-auto'>
            <h2 className="text-4xl text-primary text-center mt-5">Apply as Contest Creator</h2>

            <form onSubmit={handleSubmit(handleCreatorApplication)} className='mt-12 p-4 text-black'>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-12'>
                    ...
                    <fieldset className="fieldset">
                        ...
                        <input {...register('name')} defaultValue={dbUser?.name} className="input w-full" />

                        <label className="label">Email</label>
                        <input {...register('email')} defaultValue={dbUser?.email} className="input w-full" />
                    ...
                    <fieldset className="fieldset">
                        ...
                        <input type="number" {...register('experience')} className="input w-full" />
                    ...
                    <fieldset className="fieldset md:col-span-2">
                        ...
                        <label className="label">Why do you want to be a Contest Creator?</label>
                        <textarea {...register('motivation')} className="textarea w-full" />

                        <label className="label">Estimated Monthly Contests</label>
                        <input type="number" {...register('monthlyContests')} className="input w-full" />
